...
function Delete({ ruteId, reload }: { ruteId: Number, reload: Function }) {
    ...
    const handleDelete = async (ruteId: number) => {
        setIsLoading(true)
        handleClose()
        await axios.delete(`/admin/api/rute/${ruteId}`)
        reload()
        setIsLoading(false)
        Swal.fire({
            position: 'top-end',
        ...
        <div>
            ...
            <Modal
                ...
                <Modal.Footer>
                    ...
                    <button type="button" className="btn btn-danger light" onClick={() => handleDelete(Number(ruteId))}>Ya, Hapus</button>
                </Modal.Footer>
            </Modal>
        </div>
    )
