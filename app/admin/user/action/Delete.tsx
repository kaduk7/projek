...
function Delete({ userId, reload, userFoto }: { userId: Number, reload: Function, userFoto: String }) {
    ...
    const handleDelete = async (userId: number) => {
        setIsLoading(true)
        handleClose()
        await axios.delete(`/admin/api/user/${userId}`)
        await supabase.storage
            .from(supabaseBUCKET)
            .remove([`foto-user/${userFoto}`]);
        reload()
        setIsLoading(false)
        Swal.fire({
            position: 'top-end',
        ...
        <div>
            ...
            <Modal
                ...
                <Modal.Footer>
                    ...
                    <button type="button" className="btn btn-danger light" onClick={() => handleDelete(Number(userId))}>Ya, Hapus</button>
                </Modal.Footer>
            </Modal>
        </div>
    )
